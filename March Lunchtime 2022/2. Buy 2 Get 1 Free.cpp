/*  Link :   https://www.codechef.com/LTIME106C/problems/SALE2
   ...
   It is given that the cost of 1 item is X rupees. Find the minimum money required by Chef to buy at least N items.
          ...
          4
          Explanation
          Test Case 1: Chef wants 3 items where the cost of each item is 4 rupees. To minimise the expenditure, Chef can pay for 2 items and get the third item 
          for free. This way, the total money required is 4⋅2=8 rupees.
...
int main() 
{
    ...
    while(t--)
    {
        ...
        // pairs * pay for 2 items every time * cost of 1 item + extra item * cost of 1 item
        int ans = (pairs * 2 * x) + (extra * x);
